...
void Scene_Play::init(const std::string &levelPath) {
	...
	registerAction(sf::Keyboard::T, "TOGGLE TEXTURE");
...
Vec2 Scene_Play::gridToMidPixel(float gridX, float gridy,
	...
	// should align with the bottom left of the grid cell
	return Vec2(0, 0);
}
...
void Scene_Play::loadLevel(const std::string &filename) {
	...
	// IMPORTANT: always add the CAnimation component first so that gridToMid
	// Pixel can compute correctly
	brick->addComponent<CAnimation>(m_game->assets().getAnimation("Brick"),
									true);
	brick->addComponent<CTransform>(Vec2(96, 480));
	// NOTE: You final code should position the entity with the grid x,y
	// position read from the file: brick->addComponent<CTransform>
	// (gridToMidPixel(gridX, gridy, brick);
	...
	question->addComponent<CAnimation>(
		m_game->assets().getAnimation("Question"), true);
	question->addComponent<CTransform>(Vec2(352, 480));

	// NOTE: THIS IS INCREDIBLY IMPORTANT PLEASE READ THIS EXAMPLE
	// Components are now returned as references rather than pointers
	// If you do not specify a reference variable type, it will COPY the
	// component Here is an example: This will COPY the transform into the
	// variable 'transform1' - it is INCORRECT Any changes you make to
	// transform1 will not be changed inside the entity auto transform1 =
	// entity->get<CTransform>() This will REFERENCE the transform with the
	// variable 'transform2' - it is CORRECT
...
void Scene_Play::sLifespan() {
	// TODO: Check lifespawn of entities that have them, and destroy them if
	// they go over
}
...
void Scene_Play::sCollision() {
	...
	// Also, something BELOW something else will have a y value GREATER than
	// it Also, something ABOVE something else will have a y value LESS than
	// it
...
void Scene_Play::sAnimation() {
	// TODO: Complete the Animation class code
	// first
}
